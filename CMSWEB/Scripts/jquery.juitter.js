...
(function($) {
	var conf = {},
		...
		numMSG = 2; // set the number of messages to be show
	...
	$.Juitter = {
		...
		createURL: function(){
			...
			if (filterWords.length > 0) {
				var nots = [], item = '', searchWords = filterWords.split(","), sW = ''; 
				for (var i =0; i < searchWords.length; i++) {
					item = searchWords[i];
					sW = item.split("->").length>0 ? item.split("->")[0] : item;
					nots.push(sW.replace(/\s/g,'+'));
				}
				url += '&nots=' + nots.join('+');
			}
			return url;
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<a href='http://www.twitter.com/"+item.from_user+"' target='_blank'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /></a> <span class='tweet'><a href='http://www.twitter.com/"+item.from_user+"' target='_blank'>@"+item.from_user+":</a> "+$.Juitter.textFormat(tweet)+"<span class='time'>"+item.created_at+"</span><a href='" + link + "' class='JRM' "+openLink+" target='_blank'>"+readMore+"</a></span>";
								else mHTML="<a href='http://www.twitter.com/"+item.from_user+"' target='_blank'>@"+item.from_user+":</a> "+$.Juitter.textFormat(tweet)+" -| <span class='time'>"+item.created_at+"</span> |-  <a href='" + link + "' "+openLink+" target='_blank'>"+readMore+"</a>";
		...
		textFormat: function(texto){
			...
			var exp = /[\#]+([A-Za-z0-9-_]+)/ig;
			texto = texto.replace(exp,"<a href='' onmouseover='$.Juitter.start({searchType:\"searchWord\",searchObject:\"$1\"});return false;' class='hashLink'>#$1</a>"); 
			// make it bold
			if(mode=="searchWord"){
